...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for act in self.getLegalActions(state):
            if self.getQValue(state, act) == max_val:
                return act
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        self.values[(state, action)] = (1-self.alpha)*current + self.alpha*(self.discount * nextval+reward)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        current = self.getQValue(state, action)
        nextval = self.getValue(nextState)
        difference = (reward + self.discount * nextval) - current
        featureVector = self.featExtractor.getFeatures(state, action)
        for feature in featureVector:
