...
def sentence2() -> Expr:
    ...
    expr3 = ~conjoin(B, ~C) >> A
    expr4 = ~D >> C
    ans = conjoin(expr1, expr2, expr3, expr4)
    return ans
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    PacmanAliveat0 = PropSymbolExpr('PacmanAlive_0')
    PacmanAliveat1 = PropSymbolExpr('PacmanAlive_1')
    PacmanBornat0 = PropSymbolExpr('PacmanBorn_0')
    PacmanKilledat0 = PropSymbolExpr('PacmanKilled_0')
    expr1 = PacmanAliveat1 % disjoin(conjoin(PacmanAliveat0, ~PacmanKilledat0), conjoin(~PacmanAliveat0, PacmanBornat0))
    expr2 = ~conjoin(PacmanAliveat0, PacmanBornat0)
    expr3 = PacmanBornat0
    ans = conjoin(expr1, expr2, expr3)
    return ans
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    if findModel(expr) is not False:
        return False
    return True
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    expr: List[Expr] = []
    for everyexpr in itertools.combinations(literals, len(literals)-1):
        every: List[Expr] = []
        for i in everyexpr:
            every.append(~i)
        expr.append(conjoin(every))
    ans = disjoin(expr)
    return ans
    """
    util.raiseNotDefined()
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    atleast1 = atLeastOne(literals)
    atmost1 = atMostOne(literals)
    if atleast1 == atmost1:
        return atleast1
    expr = conjoin(atleast1, atmost1)
    return expr
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def check_index(x: int, y: int, walls_grid: List[List[bool]] = None):
    if x < 0 or x >= len(walls_grid) or y < 0 or y >= len(walls_grid[0]):
        return False
    else:
        return True
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    for x, y in all_coords:
        expr = PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t)
        pacphysics_sentences.append(expr)

    Axiom2: List[Expr] = []
    for x, y in non_outer_wall_coords:
        Axiom2.append(PropSymbolExpr(pacman_str, x, y, time=t))
    pacphysics_sentences.append(exactlyOne(Axiom2))

    Axiom3 = [PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), PropSymbolExpr('East', time=t),
              PropSymbolExpr('West', time=t)]
    pacphysics_sentences.append(exactlyOne(Axiom3))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    Note:
        ...
        - the successorAxioms should be allLegalSuccessorAxioms where needed
         需要时，后继公理应该是所有合法的后继公理
    Return:
        - a model where Pacman is at (x1, y1) at time t = 1
        - a model where Pacman is not at (x1, y1) at time t = 1
        - 一个模型，其中 Pacman 在时间 t = 1 时位于 (x1, y1)
        - 一个模型，其中 Pacman 在时间 t = 1 时不在 (x1, y1)
    """
    ...
    expr0 = conjoin(conjoin(KB), PropSymbolExpr(pacman_str, x1, y1, time=1))
    expr1 = conjoin(conjoin(KB), ~PropSymbolExpr(pacman_str, x1, y1, time=1))
    model0 = findModel(expr0)
    model1 = findModel(expr1)
...
def positionLogicPlan(problem) -> List:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    actionAT0 = [PropSymbolExpr('North', time=0), PropSymbolExpr('South', time=0), PropSymbolExpr('East', time=0),
                 PropSymbolExpr('West', time=0)]
    KB.append(exactlyOne(actionAT0))
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0))
    for posi in all_coords:
        dx = posi[0]
        dy = posi[1]
        if (not posi in walls_list) and not (dx, dy) == (x0, y0):
            KB.append(~PropSymbolExpr(pacman_str, dx, dy, time=0))

    for t in range(1, 50):
        wherepac: List[Expr] = []
        for pos in non_wall_coords:
            wherepac.append(PropSymbolExpr(pacman_str, pos[0], pos[1], time=t))
        KB.append(exactlyOne(wherepac))
        is_arrival = PropSymbolExpr(pacman_str, xg, yg, time=t)
        for pos in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(pos[0], pos[1], t, walls_grid))
        expr = conjoin(is_arrival, conjoin(KB))
        model = findModel(expr)
        if model:
            return extractActionSequence(model, actions)
        actionATt = [PropSymbolExpr('North', time=t), PropSymbolExpr('South', time=t), PropSymbolExpr('East', time=t),
                     PropSymbolExpr('West', time=t)]
        KB.append(exactlyOne(actionATt))
...
def foodLogicPlan(problem) -> List:
    ...
    for pos in all_coords:
        dx = pos[0]
        dy = pos[1]
        if (not pos in walls_list) and not (dx, dy) == (x0, y0):
            KB.append(~PropSymbolExpr(pacman_str, dx, dy, time=0))
        if pos in food:
            KB.append(PropSymbolExpr(food_str, dx, dy, time=0))
    ...
    for t in range(1, 50):
        ...
        eat_all: List[Expr] = []
        for foodpos in food:
            eat_all.append(~PropSymbolExpr(food_str, foodpos[0], foodpos[1], time=t))

        for pos in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(pos[0], pos[1], t, walls))
            if pos in food:
                have_food = PropSymbolExpr(food_str, pos[0], pos[1], time=t)
                had_food = PropSymbolExpr(food_str, pos[0], pos[1], time=t-1)
                is_eating = PropSymbolExpr(pacman_str, pos[0], pos[1], time=t)
                still_have_food = have_food % conjoin(had_food, ~is_eating)
                KB.append(still_have_food)
        expr = conjoin(conjoin(KB), conjoin(eat_all))
        model = findModel(expr)
        if model:
            return extractActionSequence(model, actions)
