...
class Expr:
    ...
    Expr has an op (operator) and a list of args.  The op can be:
      ...
      Binary (2 arg) op:
        ...
        '<=>', '^', representing logical equality and XOR 异或
    ...
    表达式可以用运算符重载构造:如果x和y是表达式，
    x + y和x & y也是。同样，如果F和x是表达式，那么表达式也是表达式
    F (x);它通过重载Expr函数的__call__方法来工作
    在由F(x)创建的exp中，op是str 'F'，而不是F.
    ...
    警告:x == y和x != y不是表达式。原因是我们想要编写代码来测试'if x == y:'和if x == y是相同的
    作为Expr('=='， x, y)，则结果将始终为真;不是什么程序员会期望的。但我们仍然需要形成express代表等价)和逻辑不相等(或异或)。你有3个选择:
    (1) Expr('<=>'， x, y)和Expr('^'， x, y)
    注意，^在Python(以及Java和c++)中是位异或。
    (2) expr('x <=> y')和expr('x =/= y')。
    查看函数expr的doc字符串。
    (3) (x % y)和(x ^ y)
    让(x % y)表示(x <=> y)是很难看的，但是我们需要
    一些操作员使(2)工作，这似乎是最好的选择。
    ...
    警告:如果x是Expr，那么x + 1也是，因为int 1被构造函数强制转换为Expr。但1 + x是错误的，因为我不知道怎么加exp。添加__radd__方法到Expr
    没有帮助，因为int__add__仍然首先被调用。)因此,
    你应该使用Expr(1) + x，或者ONE + x，或者Expr('1 + x')。
    """
...
class PropSymbolExpr(Expr):
    ...
    Expr的扩展，用来表示一个符号。这个SymbolExpr为命名符号提供了方便，特别是那些名称表示索引值的符号(例如Position[x,y]或Fluent[t])。
    符号名称必须以大写字母开头。这个类帮助将带枚举索引的方括号添加到名称的末尾。
    """
...
def pl_true(exp, model={}):
    ...
    如果命题逻辑表达式在模型中为真，则返回真，如果为假则返回假。如果模型没有为每个命题指定值，
    这可能会返回None以表示“不明显”;即使表达式是重言式的，也可能发生这种情况
    """
