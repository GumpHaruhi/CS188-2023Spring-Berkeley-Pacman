...
def constructBayesNet(gameState: hunters.GameState):
    ...
    根据项目描述图1给出的结构构建一个空的贝叶斯网络。

     您*必须*使用此函数中的常量命名所有变量。

     在此方法中，您应该：
     - 用贝叶斯网络节点填充“变量”
     - 用贝叶斯网络中的每条边填充“边”。 我们将把每条边表示为一个元组“(from, to)”。
     - 设置每个“variableDomainsDict[var] =values”，其中“values”是“var”的可能分配的列表。
         - 每个智能体的位置都是一个元组 (x, y)，其中 x 和 y 的索引为 0
         - 每个观察到的距离都是一个嘈杂的曼哈顿距离：它是非负的并且 |obs - true| <= 最大噪声
     - 与稍后为简单起见而使用的机制相比，这使用了稍微简化的机制
    """
    # constants to use
    PAC = "Pacman"
    GHOST0 = "Ghost0"
    ...
    for x in range(0, X_RANGE):
        for y in range(0, Y_RANGE):
            agentpos.append((x, y))
    variableDomainsDict[PAC] = agentpos
    variableDomainsDict[GHOST0] = agentpos
    variableDomainsDict[GHOST1] = agentpos
    for xy0 in variableDomainsDict[PAC]:
        for xy1 in variableDomainsDict[GHOST0]:
            distance = manhattanDistance(xy0, xy1)
            noise0 = distance - MAX_NOISE
            noise1 = distance + MAX_NOISE
            if noise0 >= 0 and not noise0 in obsval:
                obsval.append(noise0)
            if not noise1 in obsval:
                obsval.append(noise1)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        它应该通过交错连接变量并消除该变量来执行推理，按照消除顺序的变量顺序。 
        有关如何使用这些函数的示例，请参阅 inferenceByEnumeration
        ...
        您需要使用 joinFactorsByVariable 连接包含变量的所有因子，
        以便自动评分器识别您执行了正确的连接和消除交错。
        ...
        如果要从中消除变量的因子只有一个无条件变量，则不应消除它，而应丢弃该因子。 
        这是因为消除的结果将为 1（您将所有无条件变量边缘化），但它不是有效因子。 所以这简化了消除结果的使用。
        ...
        提示：即使为evidenceDict 传入空字典（或无），
        BayesNet.getAllCPTsWithEvidence 也将返回所有条件概率表。 
        在这种情况下，它不会专门化 CPT 中的任何变量域。
        ...
        for eliminatedvariable in eliminationOrder:
            currentFactors, joinedFactor = joinFactorsByVariable(currentFactors, eliminatedvariable)
            if len(joinedFactor.unconditionedVariables()) != 1:
                factor = eliminate(joinedFactor, eliminatedvariable)
                currentFactors.append(factor)
        fulljoined = joinFactors(currentFactors)
        return normalize(fulljoined)
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        totalval = self.total()
        randomval = random.uniform(0, totalval)
        current_weight = 0
        for key, weight in self.items():
            current_weight += weight
            if randomval < current_weight:
                return key
        return None
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None:
            ...
            return 0.0
        trueDistancsProb = busters.getObservationProbability(noisyDistance, manhattanDistance(pacmanPosition, ghostPosition))
        return trueDistancsProb
        raiseNotDefined()
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        观察结果是嘈杂的曼哈顿与您正在追踪的幽灵的距离。

        self.allPositions 是可能的幽灵位置的列表，包括监狱位置。 您应该只考虑 self.allPositions 中的位置。
        更新模型并不完全静止：它可能取决于吃豆人的当前位置。 然而，这不是问题，因为吃豆人当前的位置是已知的
        """
        "*** YOUR CODE HERE ***"
        for eachpos in self.allPositions:
            probility = self.getObservationProb(observation, gameState.getPacmanPosition(), eachpos, self.getJailPosition())
            self.beliefs[eachpos] = probility * self.beliefs[eachpos]
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldpos in self.allPositions:
            ...
            for key in newPosDist.keys():
                new_dict[key] = new_dict[key] + newPosDist[key] * self.beliefs[oldpos]
        self.beliefs = new_dict
        self.beliefs.normalize()
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        if self.numParticles <= len(self.legalPositions):
            ...
            lenth = 0
            while lenth + len(self.legalPositions) <= self.numParticles:
                self.particles += self.legalPositions
                lenth += len(self.legalPositions)
            a = self.numParticles - lenth
            self.particles += self.legalPositions[:a]
        "*** END YOUR CODE HERE ***"
    ...
    def getBeliefDistribution(self):
        ...
        返回智能体当前的信念状态，即以所有证据和时间流逝为条件的鬼魂位置分布。 
        该方法本质上是将粒子列表转换为置信分布。
        该函数应返回归一化分布
        """
        "*** YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for part in self.particles:
            probility = self.getObservationProb(observation, gameState.getPacmanPosition(), part, self.getJailPosition())
            belief[part] += probility
        belief.normalize()
        if belief.total() == 0.0:
            self.initializeUniformly(gameState)
            return
        samples = [belief.sample() for _ in range(self.numParticles)]
