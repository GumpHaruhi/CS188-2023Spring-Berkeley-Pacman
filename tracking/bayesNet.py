...
class BayesNet(object):
    ...
    def __init__(self, variables, inputInEdges, inputOutEdges, inputVariableDomains):
        ...
        贝叶斯网的基本构造函数。
         使用constructEmptyBayesNet 获得更好的界面。

         变量：所有变量的可迭代。
         inEdges：当存在从 otherVariable 到变量的边缘时，将变量映射到 otherVariable 的字典
         outEdges：将变量映射到 otherVariable 的字典，其中存在从变量到 otherVariable 的边缘
         variableDomains：将每个变量映射到其域的字典（类似列表）。
         构造一个贝叶斯网络，其边由每个变量的 inEdges 和 outEdges 给出。
         不初始化任何变量的条件概率表。
        """
    ...
    def linearizeVariables(self):
        ...
        Useful for sampling.
        返回贝叶斯网络中的变量列表，但按线性顺序排列（图中变量的父级始终按顺序位于该变量之前）。
        对于采样很有用。
        """
    ...
    def getCPT(self, variable):
        ...
        返回变量的贝叶斯网络中的条件概率表的副本。 这被实例化为一个因素
        """
...
class Factor(object):
    ...
    def __init__(self, inputUnconditionedVariables, inputConditionedVariables, inputVariableDomainsDict):
        ...
        接受一个可迭代的 unconditionVariables、一个可迭代的 ulatedVariables 
        和一个variableDomainsDict 作为输入，作为从变量到域的映射。

         inputUnconditionVariables 是变量的迭代（表示为字符串），其中包含此因子中无条件的变量
         inputConditionedVariables 是一个变量（表示为字符串）的迭代，其中包含在此因子中条件化的变量
         inputVariableDomainsDict 是从变量到这些变量的域的字典（通常表示为列表，但可以是任何可迭代的）
         将所有可能的分配字典的概率条目初始化为 0.0
        """
    ...
    def getProbability(self, assignmentDict):
        ...
        对于因子中的所有变量，输入赋值字典应采用 {variable : variableValue} 的形式。
        assignmentDict 可以拥有比因子包含的变量更多的变量（例如，它可以对贝叶斯网络中的所有变量进行赋值）
        并且它将从此因子中选择正确的行，忽略其中未包含的变量。
        返回存储在该变量分配组合的因子中的概率条目
        """
    ...
    def __getAssignmentsInOrder(self, assignmentDict):
        ...
        用于与存储的概率字典交互的内部实用函数。
        我们希望为每个分配字典存储一个概率值，但字典不可散列，因为它们是可变的，所以我们不能有一个以字典作为键的字典。
        因此，我们将概率表存储在一个字典中，其中键是变量值的元组，没有与该值关联的变量名称。
        该函数接受一个 assignmentDict 并将其处理为一个有序的值元组，其中的值是 assignmentDict 中的赋值。
        然后我们可以使用这个元组直接索引到概率表字典中。
        请改用factor.getProbability 和factor.setProbability，以获得更好的界面
        """
    ...
    def getAllPossibleAssignmentDicts(self):
        ...
        使用此函数获取因子中包含的变量组合的每个可能赋值的赋值字典。

        返回因子包含行的所有分配字典的列表，允许您在与 getProbability 和 setProbability 结合使用时迭代因子中的每一行
        """
